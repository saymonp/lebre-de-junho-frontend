    ...
    <div class="max-w-2xl mx-auto px-4 mt-6 flex flex-col gap-6">
      ...
      <div class="bg-black/40 border border-[#DBC695]/10 rounded-xl p-4 backdrop-blur-xs">
        ...
        <div class="grid grid-cols-3 gap-2">
          ...
          <!-- A Pagar -->
          <button @click="navigateTo('/pedidos?status=pendente')" class="flex flex-col items-center gap-2 p-2 rounded-lg hover:bg-white/5 transition-colors relative cursor-pointer">
            <div class="relative">
              <span class="text-xl">🪙</span>
              <span v-if="contadoresPedidos.aPagar > 0" class="absolute -top-1.5 -right-2 bg-red-500 text-white text-[10px] font-bold px-1.5 rounded-full min-w-[16px] text-center">
          ...
          <!-- Preparando -->
          <button @click="navigateTo('/pedidos?status=preparando')" class="flex flex-col items-center gap-2 p-2 rounded-lg hover:bg-white/5 transition-colors relative cursor-pointer">
            <div class="relative">
              <span class="text-xl">⚗️</span>
              <span v-if="contadoresPedidos.preparando > 0" class="absolute -top-1.5 -right-2 bg-red-500 text-white text-[10px] font-bold px-1.5 rounded-full min-w-[16px] text-center">
          ...
          <!-- À Caminho -->
          <button @click="navigateTo('/pedidos?status=enviado')" class="flex flex-col items-center gap-2 p-2 rounded-lg hover:bg-white/5 transition-colors relative cursor-pointer">
            <div class="relative">
              <span class="text-xl">🌟</span>
              <span v-if="contadoresPedidos.aCaminho > 0" class="absolute -top-1.5 -right-2 bg-red-500 text-white text-[10px] font-bold px-1.5 rounded-full min-w-[16px] text-center">
          ...
          >
            <div>
              <div class="flex items-center gap-2 mb-1">
                ...
                <span v-if="end.padrao" class="text-[10px] bg-[#DBC695]/10 text-[#DBC695] border border-[#DBC695]/30 px-1.5 py-0.5 rounded">
                  Padrão
                </span>
              </div>
            ...
            <button @click="navigateTo(`/perfil/enderecos/${end.id}`)" class="text-zinc-500 hover:text-[#DBC695] text-xs px-1">
              ✏️
            </button>
          </div>
        </div>
